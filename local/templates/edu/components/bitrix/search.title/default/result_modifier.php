...
//5
//If indexed item is section when check for
//iblock_section.png
...
foreach($arResult["CATEGORIES"] as $category_id => $arCategory)
{
	...
	$elements = array();
	$other = array();
	$sectionsIblockIDs = array();
	$elementsIblockIDs = array();
	foreach ($arCategory["ITEMS"] as $i => $arItem)
	{
		...
		//���������� �������
		if ($arItem["MODULE_ID"] == 'iblock' && in_array($arItem['PARAM2'], $arParams['DIVIDE_SECTIONS_AND_ELEMENTS']))
		{
			if (substr($arItem["ITEM_ID"], 0, 1) === "S")
			{
				$sections[] = $i;
				$sectionsIblockIDs[] = $arItem['PARAM2'];
			}
			else
			{
				$elements[] = $i;
				$elementsIblockIDs[] = $arItem['PARAM2'];
			}
		}
		else
		{
	...
	if($sections)
	{
		$arResult["CATEGORIES"][$category_id]['ITEMS_BY_GROUPS']['SECTIONS'] = $sections;
	}

	if($elements)
	{
		$arResult["CATEGORIES"][$category_id]['ITEMS_BY_GROUPS']['ELEMENTS'] = $elements;
	}

	if($other)
	{
		$arResult["CATEGORIES"][$category_id]['ITEMS_BY_GROUPS']['OTHER'] = $other;
	}
	$sectionsIblockIDs = array_unique($sectionsIblockIDs);
	$elementsIblockIDs = array_unique($elementsIblockIDs);

	if(count($sectionsIblockIDs) == 1)
	{
		$arIblock = \Bitrix\Iblock\IblockMessageTable::getList(
			array(
				"filter" => array(
					"IBLOCK_ID" => $sectionsIblockIDs[0],
		...
		if($arIblock)
		{
			$arResult["CATEGORIES"][$category_id]['SECTIONS_TITLE'] = $arIblock['MESSAGE_TEXT'];
		}
	}

	if(count($elementsIblockIDs) == 1)
	{
		$arIblock = \Bitrix\Iblock\IblockMessageTable::getList(
			array(
				"filter" => array(
					"IBLOCK_ID" => $elementsIblockIDs[0],
		...
		if($arIblock)
		{
			$arResult["CATEGORIES"][$category_id]['ELEMENTS_TITLE'] = $arIblock['MESSAGE_TEXT'];
		}
	}
}
