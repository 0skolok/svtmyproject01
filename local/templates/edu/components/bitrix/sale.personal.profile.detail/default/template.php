...
if(strlen($arResult["ID"])>0)
{
	...
	?>
	<form method="post"  class="col-md-12 form-horizontal" action="<?=POST_FORM_ACTION_URI?>" enctype="multipart/form-data">
		<?=bitrix_sessid_post()?>
		...
		<div class="form-group">
			<label class="col-md-3 control-label"><?=Loc::getMessage('SALE_PERS_TYPE')?></label>
			<div class="col-md-9"><?=$arResult["PERSON_TYPE"]["NAME"]?></div>
		</div>		
		<div class="form-group">
			<label class="col-md-3 control-label" for="sale-personal-profile-detail-name">
		...
		foreach($arResult["ORDER_PROPS"] as $block)
		{
			if (!empty($block["PROPS"]))
			{
				...
				foreach($block["PROPS"] as $key => $property)
				{
					...
					?>
					<div class="form-group">
						<label class="col-md-3 control-label" for="sppd-property-<?=$key?>">
							<?= $property["NAME"]?>:
							<?
						...
						<div class="col-md-9">
							...
							elseif ($property["TYPE"] == "FILE")
							{
								...
								?>
								<label>
									<span class="btn-default btn-md btn">
		...
		<div class="col-md-offset-3 col-sm-9">
			<input type="submit" class="btn btn-default btn-md" name="save" value="<?echo GetMessage("SALE_SAVE") ?>">
			&nbsp;
			<input type="submit" class="btn btn-default btn-md"  name="apply" value="<?=GetMessage("SALE_APPLY")?>">
			&nbsp;
