...
$arResult["COLUMN_GROUPS"] = array(); // column index => [first group index, how much groups]
$itemsInLetter = floatval($arParams["ITEMS_IN_LETTER"]);
$itemsPerColumn = ceil( (count($arResult['SECTIONS']) + count($arResult["GROUPS"])*$itemsInLetter) / $arParams['ITEMS_PER_ROW'] );
$currentInGroupCount = 0;
$currentColumn = 0;
foreach($arResult["GROUPS"] as $gIdx => $group) {
    if ($currentInGroupCount >= $itemsPerColumn) {
        $currentColumn++;
        $currentInGroupCount = 0;
    }

    $currentInGroupCount += count($group["ELEMENTS"]);
    $currentInGroupCount += $itemsInLetter;

    if (!is_array($arResult["COLUMN_GROUPS"][$currentColumn])) {
        $arResult["COLUMN_GROUPS"][$currentColumn] = array(
            'FIRST' => $gIdx,
            'LENGTH' => 0
        );
    }
    $arResult["COLUMN_GROUPS"][$currentColumn]['LENGTH'] ++;
}
