...
function JsSuggestSale(oHandler, sParams, sParser, domain, ssubmit)
{
	...
	{
		t.Destroy();
		t.oDiv = document.body.appendChild(document.createElement("ul"));
		t.oDiv.id = t.oObj.id+'_div';

		t.oDiv.className = "dropdown-menu";
        t.oDiv.style.display = 'block';

		t.aDiv = t.Print(result);
		var pos = TCJsUtils.GetRealPos(t.oObj);
        t.oDiv.style.width = t.oObj.clientWidth + 'px';
	...
	t.Print = function(aArr)
	{
		...
		for (tmp_ in aArr)
		{
			...
			// Graph
			oDiv = t.oDiv.appendChild(document.createElement("li"));
			oDiv.id = aRes['GID'];
			...
			oSpan = oDiv.appendChild(document.createElement("a"));
			oSpan.id = oDiv.id + '_NAME';
            oSpan.href = 'javascript:void(0)';
            //oSpan.onclick = 'return false;';
