...
if (!empty($arResult["errorMessage"]))
{
	...
	{
		?><div class="alert alert-danger" role="alert"><?=$arResult["errorMessage"]?></div><?;
	}
	else
	{
			...
			<div>
				...
				if ($arParams['SELL_VALUES_FROM_VAR'] != 'Y')
				{
					if ($arParams['SELL_SHOW_FIXED_VALUES'] === 'Y')
					{
						...
						<div class="row">
							<div class="col-xs-12">
								<h4><?= Loc::getMessage("SAP_FIXED_PAYMENT") ?></h4>
								<div class="sale-acountpay-fixedpay-container">
									<?
									foreach ($arParams["SELL_TOTAL"] as $valueChanging)
									{
										?>
										<div class="btn btn-default sale-acountpay-fixedpay-item">
						...
						<div class="col-xs-12 form-horizontal">
							...
							<div class="" style="max-width: 200px;">
								<div class="form-group" style="margin-bottom: 0;">
									...
									$labelWrapper = "<label class='control-label input-lg col-sm-3'>".$tempCurrencyRow."</label>";
				...
				else
				{
					...
					<div class="row">
						<input type="hidden" name="<?=CUtil::JSEscape(htmlspecialcharsbx($arParams["VAR"]))?>"
							class="form-control sale-acountpay-input"
				...
				<div class="row">
					<div class="col-xs-12">
						<h4><?=Loc::getMessage("SAP_TYPE_PAYMENT_TITLE")?></h4>
						<div>
									...
									foreach ($arResult['PAYSYSTEMS_LIST'] as $key => $paySystem)
									{
										...
										<div class="sale-acountpay-pp-company col-lg-3 col-sm-4 col-xs-6 <?= ($key == 0) ? 'bx-selected' :""?>">
											<div class="checkbox">
												...
												{
													?><img src="<?=$paySystem['LOGOTIP']?>" alt="" width="95" height="45"><?
												}
				...
				<div class="row">
					<div class="col-xs-12">
						<a href="" class="btn btn-default btn-md sale-account-pay-button"><?=Loc::getMessage("SAP_BUTTON")?></a>
					</div>
				</div>
			</div>
		</div>
