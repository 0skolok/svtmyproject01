...
$arResult["ORDER_PROPS_GROUPS"] = array();
foreach ($arResult["ORDER_PROPS"] as $id => $prop)
{
	if ($prop["SHOW_GROUP_NAME"] == "Y")
	{
		$arResult["ORDER_PROPS_GROUPS"][$prop['GROUP_NAME']] = array();
	}

	$arResult["ORDER_PROPS_GROUPS"][$prop['GROUP_NAME']][] = $id;
}

foreach ($arResult["ORDER_PROPS_GROUPS"] as $gr)
{
	$last = end($gr);
	$arResult["ORDER_PROPS"][$last]["LAST_IN_GROUP"] = 'Y';
}
...
//	$basketRefs = array();
	...
	$noPict = array(
		'SRC' => $this->GetFolder().'/images/no-photo.png'
	);
