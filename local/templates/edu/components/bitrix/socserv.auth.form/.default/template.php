...
<div class="bx-socserv-auth-form">
...
<?if(($arParams["~CURRENT_SERVICE"] <> '') && $arParams["~FOR_SPLIT"] != 'Y'):?>
    <script type="text/javascript">BX.ready(function(){BxShowAuthService('<?=CUtil::JSEscape($arParams["~CURRENT_SERVICE"])?>', '<?=$arParams["~SUFFIX"]?>')});</script>
<?endif?>
...
<form method="post" name="bx_auth_services<?=$arParams["SUFFIX"]?>" target="_top" action="<?=$arParams["AUTH_URL"]?>">
    <fieldset>
        <?if($arParams["~SHOW_TITLES"] != 'N'):?>
            <legend><?=GetMessage("socserv_as_user")?></legend>
            <p><?=GetMessage("socserv_as_user_note")?></p>
        <?endif;?>
        <?if($arParams["~FOR_SPLIT"] != 'Y'):?>
            <div class="bx-auth-services clearfix">
                <?foreach($arParams["~AUTH_SERVICES"] as $service):?>
